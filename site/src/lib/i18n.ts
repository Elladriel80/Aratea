...
export const fr = {
  ...
  mission: {
    ...
    p1: "Tout repose sur une condition non négociable : établir un edge prouvable sur la probabilité d'événements climatiques, pour la mutuelle future. Kalshi est un terrain d'entraînement — utile et auditable — pas le but. Sans cet edge, une mutuelle price mal ses contrats. Aucun capital réel tant que les probabilités ne sont pas prouvées.",
  ...
  roadmapPage: {
    ...
    title: "Roadmap",
    intro: "La preuve de l'edge reste ouverte ; la Phase 2 a avancé en parallèle pendant qu'on collecte les données. Pas de date promise, pas d'argent réel tant que les probabilités ne sont pas prouvées.",
    phases: [
      { when: "Phase 1", tag: "preuve ouverte", h: "Predictor + contrats", items: [
        "Kalshi est un terrain d'entraînement : on juge les probabilités au Brier, pas au P&L papier.",
        "Token AUG-POC + registre de rounds (Foundry) live sur testnet ; dashboard read-only en ligne.",
        "Aucun edge revendiqué. Aucun capital réel tant que l'edge de probabilités n'est pas prouvé.",
        "Sortie attendue : un go/no-go écrit sur les probabilités, pas une stratégie pour gagner de l'argent.",
      ] },
      { when: "Phase 2", tag: "avancée", h: "DAO MVP testnet", items: [
        "Gouvernance et règles avancées sur testnet — exprès, pendant la collecte de données. Ce n'est pas la preuve.",
        "Gouvernance par token (vote par token complet, cap anti-cartel).",
        "Oracle aggregator v1 ; la mutuelle (phase 3) n'est pas lancée.",
      ] },
  ...
  status: {
    ...
    contractsP: "Token AUG-POC et registre de rounds sous Foundry, tests et analyse statique en CI. Phase 2 (gouvernance, règles) avancée sur testnet Arbitrum Sepolia — en parallèle de la collecte, ce n'est pas la preuve de l'edge.",
  ...
  approach: {
    ...
    intro: "La preuve Phase 1 reste ouverte ; la Phase 2 a avancé en parallèle. On juge les probabilités au Brier, pas au P&L. Aucun capital réel tant que l'edge n'est pas prouvé.",
    p1H: "Prédire", p1P: "Un ensemble de modèles météo confronté au marché, scoré au Brier. Kalshi entraîne l'exercice ; le but est un edge de probabilités pour la mutuelle, pas de gagner de l'argent.",
  ...
  roadmap: {
    ...
    p1H: "Predictor + contrats", p1P: "Preuve encore ouverte : des probabilités meilleures que le marché, mesurées au Brier, en paper-only. Pas d'argent réel.",
    p2H: "DAO MVP testnet", p2P: "Tokens et règles avancés sur testnet, en parallèle de la collecte. Ce n'est pas la preuve.",
    p3H: "Mutuelle paramétrique", p3P: "Pas lancée. Pool de mutualisation, pricing on-chain, règlement par oracle.",
    p4H: "Couche data DePIN", p4P: "Stations météo rétribuées, oracle natif sur certaines zones.",
    current: "preuve ouverte", parallel: "avancée en parallèle", cta: "Lire la roadmap complète",
  },
  live: {
    ...
    notDeployed: "Chiffres on-chain indisponibles pour le moment — le token AUG-POC est sur testnet Arbitrum Sepolia.",
...
export const en: Dictionary = {
  ...
  mission: {
    ...
    p1: "Everything rests on one non-negotiable condition: a provable edge on the probability of climate events, for the future mutual. Kalshi is a training ground — useful and auditable — not the goal. Without that edge, a mutual misprices its contracts. No real capital until the probabilities are proven.",
  ...
  roadmapPage: {
    back: "Back to home",
    title: "Roadmap",
    intro: "The Phase 1 proof is still open; Phase 2 moved forward in parallel while data is collected. No promised dates, no real money until the probabilities are proven.",
    phases: [
      { when: "Phase 1", tag: "proof open", h: "Predictor + contracts", items: [
        "Kalshi is a training ground: we judge probabilities by Brier, not by paper P&L.",
        "AUG-POC token + round registry (Foundry) live on testnet; read-only dashboard online.",
        "No edge claimed. No real capital until a probability edge is proven.",
        "Expected output: a written go/no-go on the probabilities — not a strategy to make money.",
      ] },
      { when: "Phase 2", tag: "advanced", h: "DAO MVP testnet", items: [
        "Governance and rules advanced on testnet — on purpose, while data is collected. That is not the proof.",
        "Token governance (one vote per whole token, anti-cartel cap).",
        "Oracle aggregator v1; the mutual (phase 3) is not launched.",
      ] },
  ...
  status: {
    ...
    contractsP: "AUG-POC token and round registry built with Foundry, covered by tests and static analysis in CI. Phase 2 (governance, rules) has advanced on Arbitrum Sepolia testnet — in parallel with data collection, not as the proof of an edge.",
  ...
  approach: {
    ...
    intro: "The Phase 1 proof is still open; Phase 2 moved forward in parallel. We judge probabilities by Brier, not by P&L. No real capital until an edge is proven.",
    p1H: "Predict", p1P: "An ensemble of weather models tested against the market, scored with Brier. Kalshi trains the exercise; the goal is a probability edge for the mutual, not making money.",
  ...
  roadmap: {
    ...
    p1H: "Predictor + contracts", p1P: "Proof still open: better probabilities than the market, measured by Brier, paper-only. No real money.",
    p2H: "DAO MVP testnet", p2P: "Tokens and rules advanced on testnet, in parallel with data collection. That is not the proof.",
    p3H: "Parametric mutual", p3P: "Not launched. Mutualization pool, on-chain pricing, oracle settlement.",
    p4H: "DePIN data layer", p4P: "Rewarded weather stations, native oracle on selected areas.",
    current: "proof open", parallel: "advanced in parallel", cta: "Read the full roadmap",
  },
  live: {
    ...
    notDeployed: "On-chain figures unavailable right now — the AUG-POC token is on Arbitrum Sepolia testnet.",
